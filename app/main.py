...
@app.post(
    "/target/{targetId}/comments", responses={200: {"description": "Comment created."}}
)
def add_comment(
    comment: NewComment,
    ...
    if not comment.textFr and not comment.textEn:
        raise HTTPException(
            status_code=422, detail="A text is required at least in one language."
        )

    if comment.publishedAt and not is_timestamp(comment.publishedAt):
        raise HTTPException(
            status_code=422, detail="The date field is not a valid timestamp."
        )
